...
def coacd(in_file: str, out_file: str,
    ...
    try:
        ...
        # FIXME(ycho): hardcoded directory
...
def scene_to_mesh_with_texture(m: trimesh.Scene):
    if not isinstance(m, trimesh.Scene):
        m = trimesh.Scene([m])

    visuals = []
    for key in m.graph.nodes_geometry:
        xfm, name = m.graph[key]
        part = m.geometry[name]
        if isinstance(part.visual,
                      trimesh.visual.TextureVisuals):
            if part.visual.uv is not None:
                # == Load from properly textured mesh ==
                visuals.append(part.visual)
            else:
                # == Try loading colors from materials specification ==
                if isinstance(part.visual.material,
                              trimesh.visual.material.SimpleMaterial):
                    colors = part.visual.material.diffuse[:3]
                elif isinstance(part.visual.material,
                                trimesh.visual.material.PBRMaterial):
                    material = part.visual.material.to_simple()
                    colors = material.diffuse[:3]

                fcol = np.broadcast_to(colors,
                                       part.faces.shape)
                vcol = np.broadcast_to(colors,
                                       part.vertices.shape)

                if True:
                    # At least two pixels are required,
                    # to avoid "bleeding" into the other textures.
                    image = np.zeros((2, 2, 3),
                                     dtype=np.uint8)
                    image[..., :3] = colors
                    uv = np.zeros((len(part.vertices), 2)) + 0.5
                    image = Image.fromarray(image)
                    mat = trimesh.visual.material.SimpleMaterial(image=image)
                    visual = trimesh.visual.TextureVisuals(material=mat, uv=uv)
                else:
                    visual = (trimesh.visual.ColorVisuals(part,
                                                          face_colors=fcol,
                                                          vertex_colors=vcol)
                              .to_texture()
                              )
                visuals.append(visual)
        elif isinstance(part.visual,
                        trimesh.visual.ColorVisuals):
            # == Try loading colors from mesh face ==
            visuals.append(part.visual)

    out = m.dump(concatenate=True)
    if len(visuals) > 0:
        visuals = visuals[0].concatenate(visuals[1:])
        out.visual = visuals
    return out


def internal_face_mask(m: str, rays: int = 2048, maxq: float = 1e-3):
    ms = pymeshlab.MeshSet()
    ms.load_new_mesh(m)
    ...
    ms.compute_selection_by_scalar_per_face(
        maxq=maxq
    )
    ...
    if True:
        ...
        ms.save_current_mesh('/tmp/docker/out.obj', save_textures=True)
