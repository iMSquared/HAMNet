...
def matrix_from_pose_tq(tq: th.Tensor,
                        out: Optional[th.Tensor] = None):
    return matrix_from_pose(tq[..., :3], tq[..., 3:7], out)
...
def unsqueeze_as(a, b):
    d = max(0, len(b.shape) - len(a.shape))
    # return a[(...,), (None,) * d]
    return a.reshape(*a.shape, *((1,) * d))


def rejection_sample(sample_fn,
                     accept_fn,
                     batched: bool = True,
    ...
    """
    Arg:
        sample_fn: () -> batched?(R,N,...):(M,...)
        accept_fn: () -> batched?(R,):(M,)
        count: Target count (=N in batched case); unbatched only
        eps:   Fudge factor to avoid sampling from zeros; unbatched only

    Return:
        (N ...) if batched else (count ...)

    TODO(ycho): implement looping / iteration to
    ensure all buffers are filled (maybe)
    """

    # R x N x D... if batched; else MxD...
    if isinstance(sample_fn, th.Tensor):
        p = sample_fn
    else:
        p = sample_fn()  

    # R x N if batched; else M
    if isinstance(accept_fn, th.Tensor):
        mask = accept_fn
    else:
        mask = accept_fn(p)  

    if batched:
        # With oversampling logic
        if sample:
            n = n_per_batch
            if n is None:
                n = 1
            
            prob = einops.rearrange(mask.float()+eps,
                                    's ... -> (...) s')
            index = th.multinomial(prob, n,
                                   replacement=(n>prob.shape[1])
                                   ).swapaxes(0,1) # 8,n -> n,8
            index = index.reshape(n, *mask.shape[1:])
        else:
            # 1xN
            if n_per_batch is None:
                index = th.argmax(mask.float(), dim=0, keepdim=True)
            else:
                index = th.topk(mask.float(), k=n_per_batch, dim=0,
                                largest=True,
                                sorted=False)
        index = unsqueeze_as(index, p)
        out = th.take_along_dim(p, index, dim=0)
        if n_per_batch is None:
        ...
        index = th.multinomial(mask + eps, count, replacement=True)
        index = index.reshape(
            (...,) + (None,) * (len(p.shape) - len(index.shape)))
        if aux is not None:
            aux['index'] = index
        return th.take_along_dim(p, index, dim=0)
...
# NOTE(ycho): this function is wrong
# @th.jit.script
# def axa_from_quat_alt(x: th.Tensor):
...
#     axis = th.nn.functional.normalize(x[..., 0:3])
#     half_angle = th.acos(
#         th.clamp(x[..., 3:], -1.0, 1.0))
...
def transform_point(T: th.Tensor, p: th.Tensor,
                    out: Optional[th.Tensor] = None):
    return th.einsum('...ij,...j->...i',
                     T[..., :3, :3], p) + T[..., :3, 3]


def rot6_from_quat(q: th.Tensor):
    return matrix_from_quaternion(q)[..., :, :2].reshape(
        *q.shape[:-1], 6)


def quat_from_rot6(r: th.Tensor):
    R = th.zeros((*r.shape[:-1], 3, 3),
                 dtype=th.float32,
                 device=r.device)
    R[..., :, :2] = r.reshape(*r.shape[:-1], 3, 2)
...
def check_xfm_p():
    T = th.randn(size=(4, 4))  # ...ij: [...]=6, i=4, j=4
    p = th.randn(size=(7, 6, 3,))  # ...j -> [...]=(7,6), j=3
    q = transform_point(T, p)
    print(q.shape)
...
def check_rot6():
    print('cycle')
    q0 = random_quat(512)
    q1 = quat_from_rot6(rot6_from_quat(q0))
    print(quat_diff_rad(q0, q1).max(),
          quat_diff_rad(q0, q1).min()
          )
    ...
    # COMPARE R6
    # r0 = th.randn(size=(512, 6))
    # r1 = rot6_from_quat(quat_from_rot6(r0))
    # q0 = quat_from_rot6(r0)
    # q1 = quat_from_rot6(r1)
    # print((r0 - r1).abs().max())
