...
def randu(lo, hi, *args, **kwds):
    """ random uniform tensor within (lo,hi) range """
    return th.rand(*args, **kwds) * (hi - lo) + lo


def randu_like(lo, hi, *args, **kwds):
    """ random uniform tensor within (lo,hi) range """
    return th.rand_like(*args, **kwds) * (hi - lo) + lo
...
def masked_var_mean(
    ...
    # WARN(ycho): dim default being 0 is very dangerous...
...
def tile_data(data: Union[th.Tensor, np.ndarray],
              total: int,
              num_repeat: Optional[int] = None) -> Union[th.Tensor, np.ndarray]:
    l = len(data)
    if num_repeat is None:
        r = total // l
    else:
        r = num_repeat
    tiled = einops.repeat(data, 'b ... -> (r b) ...',
                                        r=r)
    remain = total - (l * r)
    if isinstance(tiled, np.ndarray):
        return np.concatenate([tiled, data[:remain]], 0)
    elif isinstance(tiled, th.Tensor):
        return th.cat([tiled, data[:remain]], 0)
