...
class AutoWindow:
    def __init__(self, **kwds):
        ...
        self.vis = vis
        self.key_cbs = {}
        self.set_key_cb('space',
                        lambda: self.state.__setitem__('next', True))

    def set_key_cb(self, key: str, cb: Callable):
        # These should cover the _majority_
        # of cases.
        if isinstance(key, str):
            if len(key) == 1:
                key = gui.KeyName(ord(key.lower()))
            else:
                key = getattr(gui.KeyName, key.upper())
        self.key_cbs[key] = cb
...
class Window:
    def __init__(self, **kwds):
        ...
        if True:
            ...
            margin = 0.5 * em
            self.panel = gui.Vert(0.5 * em * 4, gui.Margins(margin))
        ...
        slider.set_limits(0, 128)
        slider.set_on_value_changed(self._on_slider)
        self.slider=slider
        self.window.add_child(slider)
    ...
    def add_voxel_grid(self, key: str, voxel, **kwds):
        ...
        # NOTE(ycho): cubify() is a really weird operation
    ...
    def set_on_slider(self, idx_cb, vmax:int):
        self.slider.set_limits(0,vmax)
        self._idx_cb = idx_cb
    ...
    def _on_slider(self, new_val) -> int:
        if self._idx_cb is not None:
            self._idx_cb(new_val)
            return gui.Widget.EventCallbackResult.HANDLED
        return gui.Widget.EventCallbackResult.IGNORED
