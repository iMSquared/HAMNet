...
class NormalizeEnv(WrapperEnv):
    ...
    def __init__(self,
        ...
        # NOTE(ycho): not necessarily always correct
    ...
    def _log(self):
        ...
        if cfg.norm.normalize_rew:
            num_stat = (1 if cfg.norm.sum_rew else self.num_rew)
            for i in range(num_stat):
                if cfg.norm.per_type:
                    for j in range(cfg.norm.num_type):
                        self._add_scalar(F'log/ret_rms/{i:02d},{j:02d}/mean',
                                         self.normalizer.ret_rms.mean[..., i, j])
                        self._add_scalar(F'log/ret_rms/{i:02d},{j:02d}/var',
                                         self.normalizer.ret_rms.var[..., i, j])
                else:
                    self._add_scalar(F'log/ret_rms/{i:02d}/mean',
                                     self.normalizer.ret_rms.mean[..., i])
                    self._add_scalar(F'log/ret_rms/{i:02d}/var',
                                     self.normalizer.ret_rms.var[..., i])
    ...
    def step(self, action: th.Tensor):
        ...
        out = self.normalizer.step(super().step,
                                   self.env.buffers['step'],
                                   w_action)

        # Periodically save normalization stats.
        # NOTE(ycho): only enabled if `path` is not None
