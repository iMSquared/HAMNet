...
def apply_domain_randomization(gym, env: int, actor_handle: int,
    ...
    # NOTE(ycho): `scale` apparently does not work for now.
    # NOTE(ycho): actually, `scale` WILL NEVER WORK.
    ...
    if change_scale:
        ...
        #print(F'<change_scale> rel_scale={scale}')
    ...
    if enable_mass:
        ...
        for p in prop:
            p.mass = mass
            if enable_com:
                com = np.asarray([p.com.x, p.com.y, p.com.z])
                n_com = (com + np.random.normal(scale=com_dr_scale,
                                                size=(3,)))
                p.com = gymapi.Vec3(*(n_com.tolist()))
                out['com'] = n_com
    ...
    # TODO(JH): Switch for firction DR and its range should be added as config
...
def get_default_sim_params(dt: float = 1.0 / 240.0,
    ...
    # TODO(ycho): Consider also randomizing gravity?
    ...
    params.physx.bounce_threshold_velocity = (
        2 * 9.81 * dt / substeps)
    # params.physx.num_threads = 64 # unused in GPU mode
...
def create_camera(height: int, width: int,
    ...
    # TODO(ycho): figure out where to place the cameras,
    ...
    # NOTE(ycho): this assert fails if
...
def unproject(gym, sim, envs, cams, depth_image: th.Tensor,
    ...
    # FIXME(ycho): This routine assumes that all cameras
    ...
    # NOTE(ycho): this is not _actually_ the
