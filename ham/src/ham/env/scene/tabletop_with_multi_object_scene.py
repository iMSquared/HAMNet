...
from ham.env.common import create_camera, apply_domain_randomization
from ham.data.transforms.io_xfm import load_mesh
from ham.util.torch_util import dcn

from ham.env.scene.common import load_objects
...
class TableTopWithMultiObjectScene(TableTopScene):
    ...
    def reset(self, gym, sim, env,
        ...
        # WARN(ycho): may not be correct, like
        # > clearing prv_ids
    ...
    def create_actors(self, gym, sim, env,
        ...
        for i, key in enumerate(keys[:cfg.num_obj_per_env]):
            ...
            object_actor = gym.create_actor(
                ...
                F'object-{i:02d}',
                env_id,
                # NOTE(ycho): I believe `-1` disables self collision
                -1)
