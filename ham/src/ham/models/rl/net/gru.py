...
from ham.models.rl.net.base import AggregatorBase
...
class MultiLayerGRUAggNet(nn.Module):
    ...
    def __init__(self, cfg: Config):
        super().__init__()
        self.cfg = cfg
        num_act = np.prod(cfg.dim_act)
        dim_in: int = (cfg.dim_obs + num_act)
        self.dim_in = dim_in
...
class GRUAggNet(nn.Module, AggregatorBase):
    ...
    class Config(AggregatorBase.Config):
        # TODO(JH): dim act = to disalbe action concat
        dim_obs: Tuple[int, ...] = ()
        dim_act: Tuple[int, ...] = ()
        dim_out: int = -1
    ...
    def __init__(self, cfg: Config):
        ...
        # FIXME(ycho): currently can only handle
        # flattened obs,act !!!!
