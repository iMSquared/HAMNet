...
from ham.util.torch_util import masked_var_mean
...
def update_from_moments(
    ...
    # NOTE(ycho): without the in-place __setitem__,
...
class ConstantMeanStd(nn.Module):
    ...
    def extra_repr(self):
        return super().extra_repr() + F'shape={self.mean.shape}'
    ...
    def __init__(self,
        ...
        self.register_buffer('var',
                             th.ones(shape, dtype=th.float, device=device,
                                     requires_grad=False))
        self.__each = each_count
        if each_count:
            count_shape = shape
        else:
            count_shape = ()
        self.register_buffer('count',
                             th.full(count_shape, epsilon, dtype=th.float,
